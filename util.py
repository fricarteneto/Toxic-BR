...
def read_corpus(data_df: pd) -> pd:
    data = pd.read_csv(data_df,encoding='utf8', index_col=0, sep=';')
    return data
...
def remove_AA_error_chars(text):
    return re.sub(r'AA+', '', text)
...
def preprocessing(data: list) -> list:
    for text in data:
        text = remove_AA_error_chars(text)
        text = norm.normalise(text)
        #remove term eleicoes
        text = re.sub(r'eleicoes+', '', text.lower())
    return data
